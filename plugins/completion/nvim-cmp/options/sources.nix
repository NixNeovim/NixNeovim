...
let
  ...
  mkSourceOption = name: attr:
    mkOption {
      ...
      description =
        "Module for the ${name} (${attr.package}) source for nvim-cmp";
      default = { };
    };
  ...
  sourcesSet = {
    ...
    crates = {
      package = crates-nvim;
      extraConfig = "require('crates').setup()";
    };
    dap = { package = cmp-dap; };
  ...
  activated = cfg:
    filterAttrs (name: attrs: cfg.${name}.enable)
    plugins; # filter activted sources
...
in {
  ...
  packages = cfg-sources:
    mapAttrsToList (name: attrs: attrs.package)
    (activated cfg-sources); # # return packages of activated sources

  # list of extra config that sources define/require
  config = cfg-sources:
    mapAttrsToList (name: attrs: attrs.extraConfig)
    (activated cfg-sources); # # return packages of activated sources
