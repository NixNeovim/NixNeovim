...
in with helpers; {
  ...
  config = let
    pluginOptions = {
      highlight = {
        inherit (cfg) enable;
        disable = if (cfg.disabledLanguages != [ ]) then
          cfg.disabledLanguages
        else
          null;

        custom_captures =
          if (cfg.customCaptures != { }) then cfg.customCaptures else null;
      };
  ...
  in mkIf cfg.enable {
    programs.nixvim = {
      extraConfigLua = let
        runtimePath = optionalString (cfg.parserInstallDir != null)
