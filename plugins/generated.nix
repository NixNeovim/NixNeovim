...
let
  ...
  fillAttrs = { name, packageName ? name, setup ? false }: {
    inherit name packageName setup;
  };

in with helpers; {
  imports = lib.forEach plugs (p:
    let
      ...
      setupString =
        if plugin.setup then "require('${plugin.name}').setup()" else "";
    ...
    in mkLuaPlugin {
      inherit (plugin) name;
      description = "Autogenerated module for ${plugin.name}";
