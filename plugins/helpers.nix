...
rec {
  ...
  toConfigString = list:
    let filtered = filter (str: str != "") list;
    in concatStringsSep "\n" filtered;
  ...
  toLuaObject = args:
    if builtins.isAttrs args then
      ...
      in if hasAttr "__raw" filteredArgs then
        ...
        in if length listOfValues == 0 then
          "{}"
        else ''
          {
            ${concatStringsSep ",\n  " listOfValues}
  ...
  mkPlugin = { config, lib, ... }:
    { name, description, extraPlugins ? [ ], extraConfigLua ? ""
    , extraConfigVim ? "", options ? { }, ... }:
    let
    ...
    in {
      ...
      config.programs.nixvim = mkIf cfg.enable {
        ...
        extraConfigLua = if stringLength extraConfigLua > 0 then
          ''
  ...
  mkLuaPlugin = { name, description, extraPlugins, extraPackages ? [ ]
    , extraConfigLua ? "", extraConfigVim ? "", moduleOptions ? { },
    # ...
    }:
    let cfg = config.programs.nixvim.plugins.${name};

    in assert assertMsg (length extraPlugins > 0)
...

    {
      options.programs.nixvim.plugins.${name} = {
        ...
        extraLua = mkOption {
          ...
          default = "";
          description =
            "Place any extra lua code here that is loaded after 'extraConfig'";
        };
      } // moduleOptions;
  ...
  globalVal = val: if builtins.isBool val then (if !val then 0 else 1) else val;

  mkDefaultOpt = { type, global, description ? null, example ? null
    , default ? null, value ? v: (globalVal v), ... }: {
      option = mkOption {
