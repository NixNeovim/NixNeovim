...
in {
  ...
  imports = [
    (import ./plugins { inherit pkgs lib config helpers; })
  ];
  ...
  options = {
    programs.nixneovim = {
      ...
      plugins = (import ./plugins { inherit pkgs lib config helpers; }).plugins;
        # let
          # a = import ./plugins/utils/plantuml-syntax.nix { inherit pkgs lib config helpers; };
          # b = import ./plugins/utils/windows.nix { inherit pkgs lib config helpers; };
        # in {
        # plantuml-syntax = a.options;
        # windows = b.options;
      # };
    };
    ...
    in (mkIf cfg.enable (
      if isDocsBuild then { }
      else if homeManager then
        {
          ...
          programs.nixneovim.extraConfigLua = "hi";
        }
      else
        {
